...
`communityComments`、`recognizeDish`、`mealCheckins` 和 `ensureUser` 的 `config.json` 已声明所需的内容安全 OpenAPI 权限。发帖内容不再自动审核，发布后由管理员通过举报与管理后台人工维护；发布前仍需确认小程序主体具备其余相应接口权限。
...
至少使用两个微信账号验证：登录、协议版本升级、发帖、评论通知、收藏与屏蔽、举报与后台处理、食历跨月翻页、打卡编辑/删除、消息已读、调用频率限制、深色模式和字体大小。评论等保留的内容安全接口、定位授权和云存储图片必须在真机环境复测。
